...
const Teams = () => {
  ...
  const fetchTeams = async () => {
    try {
      const response = await axios.get(
        `${server}/api/projects/${projectId}/teams`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
  ...
  const fetchAllMembers = async () => {
    try {
      const response = await axios.get(
        `${server}/api/projects/${projectId}/teams/users`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
  ...
  const fetchTeamMembers = async (teamName) => {
    try {
      const response = await axios.get(
        `${server}/api/projects/${projectId}/teams/${teamName}/users`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
  ...
  const deleteUser = async (teamName, email) => {
    try {
      ...
      setAllMembers((prevAllMembers) =>
        prevAllMembers.filter(
          (member) => member.email !== email || member.team !== teamName
        )
      );
  ...
  const canShowActions =
    ...
    (userFromLocalStorage.role === "ADMIN" ||
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-wrap gap-6">
          {teams.map((team, index) => (
            <div
              key={index}
              className={`bg-white relative shadow-xl p-8 m-4 w-72 cursor-pointer ${
                selectedTeam === team.name
                  ? "rounded-bl-3xl rounded-tr-3xl border-t-4 border-green-400"
                  : "border rounded-xl flex"
              }`}
                      ...
                      {canShowActions && (
                        <td className="px-6 py-4 whitespace-nowrap">
                          <button
                            ...
                            onClick={() =>
                              handleDeleteClick(member.team, member.email)
                            }
                          >
                            Delete
                          </button>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      {(user.role === "ADMIN" ||
                        ...
                        <button
                          className="text-red-600 hover:text-red-900"
                          onClick={() =>
                            handleDeleteClick(selectedTeam, member.email)
                          }
                        >
                          Delete
                        </button>
